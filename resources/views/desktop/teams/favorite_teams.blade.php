...
@section("content")
    <div class = "container">
        ...
        @if(count($teams) >= 1)
            <table class="table table-striped table-light table-sm" width="100%">
                ...
                @foreach($teams as $team)
                    ...
                    @endphp
                    <tr>
                        <td scope="row" width="25%"><a href="{{ route("setFavoriteTeams", ["id" => $team->id]) }}"><i class="fas fa-star fa-fw" aria-hidden="true"></i></a>&nbsp;<a href="{{route("teamsDetails", ["id" => $team->id])}}"><img src="{{ $teamLogo }}" alt="team_logo">&nbsp;{{ $team->name }}</a></td>
        ...
        @else
            <span class="fw-bold">{{ \App\Http\Controllers\SoccerAPI\SoccerAPIController::translateString("application", "msg_no_favorite_teams") }}</span>
        @endif
    </div>
@endsection